...
//        addSingleValuedResPredicate(OWL_COMPLEMENT_OF);
    ...
    private void addLegacyMapping(URI uri) {
        ...
        if (uriString.startsWith(Namespaces.OWL11.toString())) {
            synonymMap.put(URI.create(Namespaces.OWL.toString() + uriString.substring(Namespaces.OWL11.toString().length(),
                                                                                      uriString.length())), uri);
        }
    }
    ...
    protected boolean isClass(URI uri) throws OWLException {
    ...
    protected boolean isObjectPropertyOnly(URI uri) throws OWLException {
    ...
    protected boolean isDataPropertyOnly(URI uri) throws OWLException {
    ...
    protected boolean isAnnotationProperty(URI uri) throws OWLException {
    ...
    protected OWLIndividual getOWLIndividual(URI uri) throws OWLException {
    ...
    protected void consumeTriple(URI subject, URI predicate, OWLConstant con) throws OWLException {
    ...
    private void incrementTripleCount() {
        count++;
        if (count % 10000 == 0) {
            logger.info("Parsed: " + count + " triples");
        }
    }
    ...
    public void endModel() throws SAXException {
        try {
            ...
            logger.info("Total number of triples: " + count);
            ...
            logger.info("Loaded " + ontology.getURI());
    ...
    private void handleStreaming(URI subject, URI predicate, URI object) throws OWLException {
        if (predicate.equals(RDF_TYPE.getURI())) {
            ...
            if (handler != null) {
                ...
                // Individual?
                addIndividual(subject);
//                System.out.println(subject + "  --- type --->   " + object);
            }
        }
        AbstractResourceTripleHandler handler = predicateHandlers.get(predicate);
        if (handler != null) {
...
//        }
    ...
    public OWLObjectPropertyExpression translateObjectPropertyExpression(URI mainNode) throws OWLException {
        ...
        if (!isAnonymousNode(mainNode)) {
            ...
            return getDataFactory().getOWLObjectPropertyInverse(otherProperty);
        }
    }
    ...
    public URI getResourceObject(URI subject, URI predicate, boolean consume) throws OWLException {
    ...
    public OWLConstant getLiteralObject(URI subject, URI predicate, boolean consume) throws OWLException {
    ...
    public boolean isTriplePresent(URI subject, URI predicate, URI object, boolean consume) throws OWLException {
        Map<URI, URI> subjPredMap = singleValuedResTriplesByPredicate.get(predicate);
        if (subjPredMap != null) {
            URI obj = subjPredMap.get(subject);
            if (consume) {
    ...
    public boolean isTriplePresent(URI subject, URI predicate, OWLConstant object, boolean consume) throws
                                                                                                    OWLException {
        Map<URI, OWLConstant> subjPredMap = singleValuedLitTriplesByPredicate.get(predicate);
        if (subjPredMap != null) {
            OWLConstant obj = subjPredMap.get(subject);
    ...
    public boolean hasPredicate(URI subject, URI predicate) throws OWLException {
