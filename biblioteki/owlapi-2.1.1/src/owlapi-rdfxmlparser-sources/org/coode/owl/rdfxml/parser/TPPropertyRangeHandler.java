    ...
    public void handleTriple(URI subject, URI predicate, URI object) throws OWLException {
        if (getConsumer().isObjectPropertyOnly(subject)) {
            ...
            if(getConsumer().isDataRange(object)) {
                ...
                logger.info("Assuming data property because range appears to be datatype: " + subject + " -> " + predicate + " -> " + object);
                addAxiom(getDataFactory().getOWLDataPropertyRangeAxiom(translateDataProperty(subject),
                                                                   translateDataRange(object)));
                consumeTriple(subject, predicate, object);
            }
                ...
                logger.info("Assuming object property because range appears to be a class: " + subject + " -> " + predicate + " -> " + object);
                translateObjectPropertyRange(subject, object, predicate);
            }
            else {
                // Right - just assume an object property!
                logger.info("Unable to determine range type.  Assuming object property: " + subject + " -> " + predicate + " -> " + object);
                translateObjectPropertyRange(subject, object, predicate);
            }
        }
    }
